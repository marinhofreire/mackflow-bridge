...
var envSchema = external_exports.object({
  CABME_BASE_URL: external_exports.string().min(1),
  CABME_APIKEY: external_exports.string().min(1),
  CABME_ACCESSTOKEN: external_exports.string().min(1),
  ZPRO_BASE_URL: external_exports.string().min(1),
  ZPRO_API_ID: external_exports.string().min(1),
  ZPRO_TOKEN: external_exports.string().min(1),
  REQUEST_TIMEOUT_MS: external_exports.string().optional(),
  LOG_LEVEL: external_exports.string().optional()
});
function getConfig(env) {
  const parsed = envSchema.parse(env);
  const timeoutMs = parsed.REQUEST_TIMEOUT_MS ? Number(parsed.REQUEST_TIMEOUT_MS) : 15e3;
  return {
    cabme: {
...
// src/worker/connectors/cabme.ts
...
async function getVehicleCategories(env) {
...
__name(getVehicleCategories, "getVehicleCategories");

// src/worker/connectors/zpro.ts
async function fetchWithTimeout2(input, init, timeoutMs) {
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), timeoutMs);
  try {
    const response = await fetch(input, { ...init, signal: controller.signal });
...
__name(timedCall, "timedCall");
async function adminSmokeHandler(c) {
  const env = c.env;
...
__name(adminSmokeHandler, "adminSmokeHandler");
...
// .wrangler/tmp/bundle-OpAHsp/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-OpAHsp/middleware-loader.entry.ts
